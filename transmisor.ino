...
void setup() {
  ...
  // Configurar la ruta principal para ingresar palabras
  server.on("/", HTTP_GET, []() {
...
void transmitirPalabra(String palabra) {
  for (int i = 0; i < palabra.length(); i++) {
    ...
    Serial.print("Transmitiendo caracter: ");
    Serial.println(caracter);

    // Transmitir el carácter en 5 bits con bit de sincronización
...
void transmitirCaracter(char caracter) {
  ...
  if (caracter < 'A' || caracter > 'Z') {
    Serial.println("Caracter inválido: " + String(caracter));
    return;
  }
  ...
  // Convertir el carácter a 5 bits (A=1, B=2, ..., Z=26)
  int valor = caracter - 'A' + 1; // A=1, B=2, ..., Z=26
